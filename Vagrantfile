...
file    = 'servers.yaml'
domain  = 'example.com'        # demain domain to use
network = '192.168.10.0/24'    # default network to use
box     = 'bento/centos-7.3'   # default image name
cpu     = 1                    # default vcpu count in int (default to undef)
memory  = 1024                 # default memory size in mb (default to undef)
boxurl  = ''                   # default url prefix (useful for private boxes)
...
begin
  ...
  domain   = settings.fetch(:domain, domain)
  network  = settings.fetch(:network, network)
  box      = settings.fetch(:box, box)
  cpu      = settings.fetch(:cpu, cpu)
  memory   = settings.fetch(:memory, memory)
  update   = settings.fetch(:update, update)
